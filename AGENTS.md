...
6. Route skill-quality, PM/UED focus group, adoption-readiness, or "is this skill good?" reviews through `sunzi-focus-group-skill-review` before final recommendation.
7. When the user asks for industry-leader review, consultation-company smoke testing, or top industry problems, use synthetic industry archetypes, ask for Top 3 Problems, select a case, then run SSR, focus-group review, and `strategy-analyst-review`.
8. Route end-to-end synthetic industry smoke tests through `sunzi-strategy-consultant` -> `sunzi-stakeholder-ssr` -> `sunzi-focus-group-skill-review` -> `strategy-analyst-review`.
9. Route review through `strategy-analyst-review` before recommending action.
10. Attach the public reference pack when the host supports references.
